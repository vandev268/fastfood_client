...
import {
  ...
  type CreateReservationBodyType,
  type UpdateReservationBodyType
...
import { useReservationDetailQuery, useUpdateReservationMutation } from '@/queries/useReservation'
...
export default function EditReservation({
  ...
  setEditReservationId: (id: number | undefined) => void
}) {
  const [tableDialogOpen, setTableDialogOpen] = useState(false)
  const [previewTable, setPreviewTable] = useState<TableType | null>(null)

  const form = useForm<UpdateReservationBodyType>({
    resolver: zodResolver(UpdateReservationBodySchema),
    defaultValues: {
      guestName: '',
      guestPhone: '',
      numberOfGuest: 1,
  ...
  useEffect(() => {
    if (reservationDetailQuery.data) {
      const { guestName, guestPhone, numberOfGuest, reservationTime, note, tableId, status, userId, table } =
  ...
  }, [reservationDetailQuery.data, form])
  ...
  const onSubmit = async (body: CreateReservationBodyType) => {
    if (updateReservationMutation.isPending || !reservationId) return
    try {
    ...
    <>
      <Dialog onOpenChange={reset} open={Boolean(reservationId)}>
        <DialogContent className='sm:max-w-[750px] max-h-screen overflow-auto' aria-describedby={undefined}>
          ...
          <DialogFooter>
            <Button type='submit' form='edit-reservation-form'>
              Sửa
            </Button>
          </DialogFooter>
        </DialogContent>
